...
$( document ).ready( () => {
    ...
    $( "#save" ).click( () => {
        ...
        if ( dob === "" || !isDate(dob, datePattern, "bdDate") ) {
            isValid = false;
            $("#dob").next().text("Please enter a valid date in MM/DD/YYYY format.");
        }
        if ( card === "" || !cardPattern.test(card)) {
            isValid = false;
            $("#card").next().text("Please enter a card number in NNNN-NNNN-NNNN-NNNN format.");
        }
        if ( expiration === "" || !isDate(expiration, expirationPattern, "exDate")) {
            isValid = false;
            $("#cc_date").next().text("Please enter valid date in MM/YYYY format");
        }
